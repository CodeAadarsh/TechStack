...
background: hover:scale-110 duration-500  bg-[#ba6993]
label: CSS
tags:
    - css
categories:
...
@each $item in $menu-items {
    .photo-#{$item} {
      background: hover:scale-110 duration-500  url('#{$item}.jpg');
    }
}
```
...
@each $id, $image in $backgrounds {
    .photo-#{$id} {
      background: hover:scale-110 duration-500  url($image);
    }
}
```
